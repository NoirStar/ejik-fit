...
def test_initial_sources_include_existing_greeting_pages_and_official_json_sources() -> None:
    ...
    assert len(seed_data.INITIAL_GREETING_SOURCES) == 69
    assert all(
    ...
    kbank = catalog_by_slug["kbank"]
    assert kbank.source_type == SourceType.PUBLIC_JSON_DETAIL
    assert kbank.connector_family == "recruiter_legacy_public_api_tech"
    assert kbank.request_method == "POST"
    assert kbank.request_body is not None
    assert kbank.request_body["pageSize"] == "100"
    assert catalog_by_slug["line-plus"].source_type == SourceType.LINE_GATSBY
    assert catalog_by_slug["hybe"].connector_family == "greeting_hybe_tech"
    assert catalog_by_slug["spoonlabs"].connector_family == (
        "greeting_spoonlabs_tech"
    )
    assert catalog_by_slug["teamsparta"].connector_family == (
        "greeting_teamsparta_tech"
    )
    assert catalog_by_slug["remember"].connector_family == (
        "ninehire_public_api_tech"
    )
    assert catalog_by_slug["payhere"].connector_family == (
        "ninehire_public_api_tech"
    )
    ...
    assert catalog_by_slug["levit"].connector_family == (
        "ninehire_public_api_tech"
    )
    ...
    liner = catalog_by_slug["liner"]
    assert liner.source_type == SourceType.HTML_LISTING_DETAIL
    ...
    estfamily = catalog_by_slug["estfamily"]
    assert estfamily.source_type == SourceType.GREETING
    assert estfamily.connector_family == "greeting_estfamily_tech"
    assert estfamily.status == SourceStatus.ALLOWED
    assert estfamily.policy_status == PolicyStatus.ALLOWED
    ...
    miridih = catalog_by_slug["miridih"]
    assert miridih.source_type == SourceType.GREETING
    assert miridih.connector_family == "greeting_tech"
    assert miridih.status == SourceStatus.ALLOWED
    assert miridih.policy_status == PolicyStatus.ALLOWED
...
def test_initial_sources_include_synopsys_korea_official_source() -> None:
    sources = {item.slug: item for item in seed_data.INITIAL_SOURCE_CATALOG}

    synopsys = sources["synopsys-korea"]
    assert synopsys.base_url == (
        "https://careers.synopsys.com/location/"
        "south-korea-jobs/44408/1835841/2"
    )
    assert synopsys.source_type == SourceType.HTML_LISTING_DETAIL
    assert synopsys.connector_family == "synopsys_talentbrew_korea_tech"
    assert synopsys.status == SourceStatus.ALLOWED
    assert synopsys.policy_status == PolicyStatus.ALLOWED
...
def test_initial_sources_include_phase_two_enterprise_sources_with_lg_api_enabled() -> None:
    ...
    assert catalog_by_slug["hyundai-mobis"].source_type == (
        SourceType.HTML_LISTING_DETAIL
    )
    assert catalog_by_slug["hyundai-mobis"].connector_family == (
...
def test_initial_sources_include_verified_fintech_and_ai_greeting_sources() -> None:
    verified_sources = {
        ...
        "kbds": "https://kbds.career.greetinghr.com/ko/openposition",
        "deepx": "https://deepx.career.greetinghr.com/ko",
        "soomgo": "https://soomgo.team/career",
    }
    catalog_by_slug = {item.slug: item for item in seed_data.INITIAL_SOURCE_CATALOG}
...
def test_initial_sources_include_verified_consumer_platform_sources() -> None:
    catalog_by_slug = {item.slug: item for item in seed_data.INITIAL_SOURCE_CATALOG}

    for slug, base_url in (
        ("kmong", "https://kmong.career.greetinghr.com/ko/home"),
        ("zigbang", "https://zigbang.career.greetinghr.com/ko/home"),
        (
            "kakao-enterprise",
            "https://careers.kakaoenterprise.com/ko/intro",
        ),
        ("11st", "https://11st.career.greetinghr.com/ko/career"),
        ("qanda", "https://recruit.mathpresso.com/ko/home"),
    ):
        source = catalog_by_slug[slug]
        assert source.base_url == base_url
        assert source.source_type == SourceType.GREETING
    ...
    for slug, base_url in (
        ...
        ("class101", "https://jobs.class101.net/career"),
    ):
        source = catalog_by_slug[slug]
        assert source.base_url == base_url
        assert source.source_type == SourceType.PUBLIC_JSON_DETAIL
    ...
    assert yanolja.source_type == SourceType.WORKDAY
    assert yanolja.connector_family == "workday_public_api_korea_tech"
    assert yanolja.request_method == "POST"
    assert yanolja.request_body == {
        "appliedFacets": {},
        "limit": 20,
        "offset": 0,
        "searchText": "",
    ...
    bunjang = catalog_by_slug["bunjang"]
    assert bunjang.base_url == "https://team.bgzt.co.kr/"
    assert bunjang.source_type == SourceType.PUBLIC_JSON_DETAIL
    assert bunjang.connector_family == "ninehire_public_api_tech"
    assert bunjang.status == SourceStatus.ALLOWED
    assert bunjang.policy_status == PolicyStatus.ALLOWED
