...
- 저장함은 현재 계정의 서버 저장 membership을 기준으로 커서 페이지를 끝까지
  이어서 조회한다. 첫 50개 이후의 저장 글도 `더 보기`로 복원된다.
- 팔로잉은 현재 계정의 author follow membership을 DB RPC에서 먼저 제한한 뒤
  커서 페이지로 조회한다. 전체 최신 피드의 첫 페이지에 없는 팔로우 글도 표시된다.
...
- 로컬 Supabase 호환 브라우저 픽스처가 익명·타 사용자 쓰기를 차단하면서 게시,
  ...
  삭제까지 한 여정으로 검증한다.
- `npm run smoke:public`은 쿠키와 쓰기 없이 홈·로그인·개인정보·검색·안전한 404,
  잘못된 인증 callback의 동일 출처 로그인 복귀, API health, Supabase 공개
  커뮤니티 read까지 8개 경계를 확인한다. `DEPLOYMENT_NOT_FOUND`, Vercel
  Authentication 보호 화면, 외부 redirect와 예상 밖 JSON을 실패로 분류한다.
