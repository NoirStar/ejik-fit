...
## 2026-07-20 출처 신뢰성 갱신

- 공고 상태의 `delayed`는 마감이 아니라 공식 출처 재확인이 지연된 상태다.
- 실행할 수 없는 출처에는 마지막 성공 시각부터 72시간의 유예 시간을 적용하고,
  이후에도 재검증하지 못한 `open` 공고만 `delayed`로 전환한다.
- 일반 검색, 기술 통계, 시장 스냅샷은 `open` 공고만 사용한다. 지연 공고 상세는
  남겨 공식 원문과 마지막 확인 시각을 확인할 수 있게 한다.
- 지연 공고가 정상 수집 목록에서 다시 발견되면 `open`으로 복구하며, 정상 목록에서
  3회 연속 보이지 않을 때만 `closed`로 전환한다.
- 넥슨 13개 법인은 일반 Playwright Chromium을 Xvfb에서 실행해 하나의 공식 목록을
  공유하고 법인별로 분리한다. 로그인, 프록시, stealth, CAPTCHA 우회는 사용하지 않는다.
- 과거 자동 접근 오류로 남은 넥슨 `blocked`/`review` 조합은 seed 시 재시도 가능한
  상태로 복구한다. 운영자가 명시적으로 설정한 `stopped` 상태는 보존한다.
- 2026-07-20 운영 재검증에서는 넥슨 공식 홈페이지가 일반 Chromium에도 HTTP 403을
  반환했다. 렌더러가 없어서가 아니라 공식 출처의 현재 접근 제한이며, 해당 출처는
  `access_limited`로 표시하고 우회 없이 다음 정기 검증을 기다린다.
...
- 현재 카탈로그는 51개 출처이며 `allowed` 44개, `needs_browser` 4개, `needs_connector` 3개다.
...
- 네이버, 카카오, LINE, Greeting 25개와 검증된 기업 공식 출처를 합쳐 44개가 `allowed` 상태다.
- 2026-07-15에는 기존 Greeting 커넥터로 목록과 상세 본문을 확인한 업스테이지,
  리벨리온, 마키나락스, 노타AI, 뤼튼테크놀로지스, 코빗, 람다256, 포트원,
  캐럿AI 9개 출처를 추가했다.
