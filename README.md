...
- 필수·우대·미분류 근거를 구분한 기술 수요와 누적 주간 추세
- 공고와 기술의 연결을 탐색하는 라이트 모드 스킬맵
- 이메일·비밀번호·닉네임 가입, 이메일 확인, 비밀번호 재설정
- 내 스택·저장 공고·지원 단계·관심 기업의 계정 동기화
...
- 이전 버전이 `localStorage`에 남긴 글과 댓글은 계정 이관이 전부 성공할 때까지
  ...
  않고 재시도할 수 있습니다.
- 이직핏 커뮤니티 가이드는 코드에 포함된 읽기 전용 예시입니다. 실제 회원 활동,
  내 글, 저장함, 팔로잉 및 반응 수에 포함되지 않습니다.
