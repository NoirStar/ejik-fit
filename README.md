...
내 경력과 기술이 어떤 커리어 방향과 이어지는지 판단하고, 그 방향과 연결된 기업
공식 채용공고와 시장 근거를 함께 확인하는 한국어 커리어 분석 서비스입니다.

현재 저장소는 운영 베타 전체 애플리케이션입니다. 여러 공식 채용 플랫폼과 공개
JavaScript 화면을 수집해 원문 스냅샷·공고 리비전·기술 근거를 저장하고, 웹과 API를
Vercel에서 제공하며 GitHub Actions가 운영 수집을 주기적으로 갱신합니다.

## 현재 제공 기능

- 기업 공식 공고 검색·필터·상세·기업별 채용 현황
- 필수·우대·조건 구분 없음 근거를 나눈 기술 수요와 누적 주간 추세
- 경력에서 이어지는 분야를 보는 커리어맵과 보조 기술 관계 보기
- 이메일·비밀번호·닉네임 가입, 이메일 확인, 비밀번호 재설정
- 내 기술·저장 공고·지원 단계·관심 기업의 계정 동기화
- 저장 검색 기반 인앱 공고 알림과 채용 일정
- 계정 커뮤니티 글·댓글·공감·저장·팔로우·신고·내보내기
- 운영 DB와 자동 연결된 수집 기업·공식 출처 공개 목록

## 데이터 저장 범위

- 실제 커뮤니티 글·댓글·공감·저장·팔로우·신고는 로그인 계정 기준으로
  Supabase에 저장됩니다. 새 커뮤니티 글을 `localStorage` 게시물로 만드는 경로는
  없습니다.
- 로그인 전 작성 중인 커뮤니티 글은 현재 탭의 `sessionStorage`에 최대 7일 동안
  임시 저장됩니다. 로그인 후 내용을 다시 확인하고 게시 버튼을 눌러야 하며,
  서버 저장에 성공한 뒤에만 초안이 삭제됩니다.
- 이전 버전이 `localStorage`에 남긴 글과 댓글은 계정 이관이 전부 성공할 때까지
  복구 영역에 보존됩니다. 일부 댓글이라도 이관에 실패하면 해당 원본을 지우지
  않고 재시도할 수 있습니다.
- 커뮤니티에는 서버에 게시된 실제 계정 글과 이전 브라우저에서 복구할 사용자 글만
  표시합니다. 초기 화면을 채우기 위한 가상 게시물은 노출하지 않습니다.
- 내 기술·커리어 조건·저장 공고·지원 단계·관심 기업은 로그인 전 브라우저에
  저장되고, 로그인 후 `user_career_states`와 병합·동기화됩니다.

## 수집 원칙

- 로그인 없이 공개된 기업 공식 채용 페이지만 수집합니다.
...
- 주간 운영 스냅샷이 충분히 쌓인 기술부터 12주 추세를 순차 공개합니다.
- 각 공식 출처의 구조 변경과 수집 지연을 계속 감시하고 커넥터를 보강합니다.
- 개인 프로필을 이용한 자동 공고 매칭은 현재 공개 범위에 포함하지 않습니다.

상세 설계와 변경 기록은 [`docs/superpowers`](docs/superpowers)에 있습니다.
