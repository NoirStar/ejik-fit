...
const fit: FitAnalyzeResponse = {
  ...
  domain_branches: [
    {
      domain: "backend",
      covered_skills: ["Java", "Kafka"],
      missing_required_skills: ["Spring"],
      missing_preferred_skills: ["Kubernetes"],
      supporting_posting_count: 12,
    },
  ],
...
describe("buildHomeFeedSnapshot", () => {
  ...
  it("keeps starter guidance out of the verified action feed", () => {
    ...
    expect(snapshot.starterGuideItems).toEqual([]);
    expect(snapshot.recommendedJobs[0]).toMatchObject({
      companyName: "토스",
    ...
    expect(snapshot.careerDirections).toEqual([
      {
        domain: "backend",
        label: "백엔드",
        coveredSkills: ["Java", "Kafka"],
        additionalRequirements: ["Spring", "Kubernetes"],
        postingCount: 12,
        confirmedCompanyCount: 1,
        representativeJob: {
          id: "job-1",
          title: "Backend Engineer",
          companyName: "토스",
  ...
  it("deduplicates direction company evidence by stable posting company slug", () => {
    const duplicatePosting = {
      ...postings.items[0],
      id: "job-2",
      company_name: "토스 주식회사",
    };
    const snapshot = buildHomeFeedSnapshot({
      postings: ready({
        total: 2,
        items: [postings.items[0], duplicatePosting],
      }),
      skillStats: ready(skillStats),
      graph: ready({
        ...graph,
    ...
    expect(snapshot.careerDirections[0].confirmedCompanyCount).toBe(1);
  });
