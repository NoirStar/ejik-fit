...
describe("JobList", () => {
  ...
  it("renders URL filters and verified requirement evidence", async () => {
    ...
    expect(screen.getByText("전체 채용공고 2건")).toBeInTheDocument();
    ...
    expect(screen.getByRole("link", { name: "검색 조건 초기화" })).toHaveAttribute(
      "href",
      "/jobs",
    );
  ...
  it("distinguishes empty, missing-stack, and API error states", async () => {
    ...
    expect(screen.getByText("조건에 맞는 채용공고가 없습니다.")).toBeInTheDocument();
