...
- 선택 주변은 직접 연결과 두 단계 보기만 제공하고 최대 30개 노드로 제한한다. 개인의 보유 기술은
  공유 가능한 시장 관계망 위에 브라우저에서 덧씌우며, 보유 기술 변경만으로 관계망을 다시 받지 않는다.
- 모바일 스킬맵은 기본적으로 페이지 스크롤을 유지한다. 사용자가 `그래프 조작`을 명시적으로 켠
  동안만 이동·핀치 확대·노드 탭을 받고, 끄면 즉시 문서 스크롤로 돌아간다.
