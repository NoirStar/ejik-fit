...
이 문서는 커리어핏의 잠긴 앱 디자인 시스템이다. 페이지별 취향을 새로 만들지 않고,
...
- 홈: **Personal Career Brief** — 커리어 방향, 연결 근거, 관련 공고와 시장 변화를 우선순위에 따라 노출한다.
- 내 커리어·시장·공고: **Workbench** — 작은 페이지 안내 뒤에 제어 영역과 주 작업 영역을 바로 둔다.
- 커리어 방향 비교: **Comparison workbench** — 같은 기준의 목록과 상세 패널로 방향을 비교하고, 기술 관계 그래프는 보조 보기로 제공한다.
- 커뮤니티: **Focused Forum** — 홈 피드와 분리하고 커리어 질문과 경험 공유에 집중한다.
...
- Navigation: **N1b product shell with visible search spine**. 브랜드 링크가 홈을 담당하며,
  내 커리어·방향 비교·채용공고·채용 시장만 주요 목적지로 둔다. 기술 관계와 커뮤니티는
  보조 탐색으로 제공한다. 데스크톱 헤더와 모바일 내비게이션은 같은 정보구조를 공유한다.
...
- 홈의 커리어 브리핑은 현재 경험에서 이어지는 방향과 추천 채용공고를 먼저 보여준다. 기술
  학습 후보나 근거 없는 준비도는 만들지 않으며, 시장 전체 수요는 별도 요약으로 구분한다.
...
- 기술 입력은 `/career`의 프로필 흐름 안에서 직무·업무·프로젝트 경험과 함께 관리한다. 빈
  입력에서 전체 카탈로그를 펼치지 않고, 검색할 때 표준 기술을 보여주며 직접 입력도 허용한다.
- 시장은 표·막대·범례를 우선하며 파스텔은 데이터 의미에만 사용한다.
- 기술 관계 보기는 그래프를 가장 넓은 주 작업 면으로 둔다. 노드 크기는 시장 수요, 채움색은
  기술 분야, 보라색 실선 테두리는 내 기술을 뜻한다. 선의 농도는 같은 공고에 함께 나온 관계
  강도만 표현하며 추천·인과·학습 순서의 의미를 부여하지 않는다.
...
- 분야 필터는 전체 상태에서 첫 분야를 누르면 해당 분야만 남기고, 이후 선택은 비교 분야를
  추가하거나 제거한다. 마지막 분야를 해제하면 전체로 돌아가며, 단일 선택은 개수 대신 분야명을
  제어에 바로 표시한다. 이 필터와 관계선·기술명 설정은 네트워크 요청, force 재가열, 카메라
  재맞춤 없이 같은 좌표에서 opacity와 작은 scale만 220ms 안에 전환한다.
